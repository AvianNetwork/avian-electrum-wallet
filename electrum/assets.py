...
from .avian import opcodes, push_script, base_encode, TOTAL_COIN_SUPPLY_LIMIT_IN_BTC, COIN, base_decode
...
def get_asset_vout_type(script: bytes) -> Optional[str]:
    ...
    avn_ptr = -1
    for op, l, ptr in ops:
        if op == opcodes.OP_AVN_ASSET:
            avn_ptr = ptr - 1
            break
    if not avn_ptr > 0:
        return None
    if script[avn_ptr+2:avn_ptr+5] == b'avn':
        avn_ptr += 5
    else:
        avn_ptr += 6
    type = bytes([script[avn_ptr]])
...
def try_get_message_from_asset_transfer(script: bytes) -> Optional[Tuple[str, Optional[int]]]:
    ...
    if type != b't':
        return None
    avn_ptr += 1
    name_len = script[avn_ptr]
    ipfs = None
    timestamp = None
    try:
        if script[avn_ptr+1+name_len+8] != 0xff:
            ipfs = script[avn_ptr+1+name_len+8:avn_ptr+1+name_len+8+34]
        if script[avn_ptr+1+name_len+8+34] != 0xff:
            timestamp = script[avn_ptr+1+name_len+8+34:avn_ptr+1+name_len+8+34+8]
...
def replace_amount_in_transfer_asset_script(script: bytes, amount: int) -> bytes:
    ...
    for op, _, ptr in ops:
        if op == opcodes.OP_AVN_ASSET:
            ...
            break
    if not avn_ptr > 0:
        raise BadAssetScript('No OP_AVN_ASSET')
    if script[avn_ptr+2:avn_ptr+5] == b'avn':
        avn_ptr += 5
    else:
        avn_ptr += 6
    ...
    avn_ptr += 1
    name_len = script[avn_ptr]
    pre_sats = script[:avn_ptr+1+name_len]
    post_sats = script[avn_ptr+1+name_len+8:]
...
def pull_meta_from_create_or_reissue_script(script: bytes) -> Dict:
    ...
    if type == b'q':
        name_len = script[avn_ptr]
        name = script[avn_ptr+1:avn_ptr+1+name_len]
        sats = script[avn_ptr+1+name_len:avn_ptr+1+name_len+8]
        divs = script[avn_ptr+1+name_len+8]
        reis = script[avn_ptr+1+name_len+8+1]
        has_i = script[avn_ptr+1+name_len+8+1+1]
        ifps = None
        if has_i != 0:
            ifps = script[avn_ptr+1+name_len+8+1+1+1:avn_ptr+1+name_len+8+1+1+1+34]
        return {
        ...
        reis = script[avn_ptr + 1 + name_len + 8 + 1]
        ifps = None
        if avn_ptr + 1 + name_len + 8 + 1 + 1 != len(script) - 1:
            ifps = script[avn_ptr + 1 + name_len + 8 + 1 + 1:avn_ptr + 1 + name_len + 8 + 1 + 1 + 34]
        return {
            'name': name.decode('ascii'),
            'sats_in_circulation': int.from_bytes(sats, 'little'),
            'divisions': divs,
...
def is_main_asset_name_good(name):
    ...
    if re.search(AVIAN_NAMES, name):
        return "Main assets cannot have Avian-like names."
