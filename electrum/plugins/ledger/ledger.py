...
from electrum.avian import int_to_hex, var_int, is_segwit_script_type, is_b58_address
...
MSG_NEEDS_FW_UPDATE_SEGWIT = _('Firmware version (or "avian" app) too old for Segwit support. Please update at') + \
                             ' https://www.ledgerwallet.com'
        ...
        if not self.preflightDone:
            try:
                ...
                if (e.sw == 0x6d00 or e.sw == 0x6700 or e.sw == 0x6702):
                    raise UserFacingException(_("Device not in avian mode")) from e
                raise e
    ...
    def parsed_string(self):
        ...
        if self.send_warning:
            line += _('It looks there is a lot of data to parse.\n'
                      'This occurs if you receive AVN from transactions\n'
                      ...
                      'Once this transaction is complete and sent, your AVN will\n'
